...
export class AlignGuidelines {
  ...
  constructor({
    ...
    if (aligningOptions) {
      this.aligningLineMargin =
        aligningOptions.lineMargin || this.aligningLineMargin;
      this.aligningLineWidth =
        aligningOptions.lineWidth || this.aligningLineWidth;
      this.aligningLineColor =
        aligningOptions.lineColor || this.aligningLineColor;
    }
  }
  ...
  private drawVerticalLine(coords: VerticalLineCoords) {
    ...
    if (
      !Keys(movingCoords).some(
        (key) => Math.abs(movingCoords[key].x - coords.x) < 0.0001
      )
    )
      return;
    this.drawLine(
  ...
  private drawHorizontalLine(coords: HorizontalLineCoords) {
    ...
    if (
      !Keys(movingCoords).some(
        (key) => Math.abs(movingCoords[key].y - coords.y) < 0.0001
      )
    )
      return;
    this.drawLine(
      ...
      const canvasObjects = this.canvas.getObjects().filter((obj) => {
        if (this.ignoreObjTypes.length) {
          return !this.ignoreObjTypes.some(
            (item) => (obj as any)[item.key] === item.value
          );
        }
  ...
  private calcCenterPointByACoords(
    coords: NonNullable<fabric.Object['aCoords']>
  ): fabric.Point {
